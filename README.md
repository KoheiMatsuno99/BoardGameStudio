...
1つ目はフロントエンドのリファクタリングである。イベントをハンドリングする上でuseStateが肥大化してしまったため、useStateのみを別のtsファイルとしてに切り離すことにした。
...
2つ目はDjango Rest FrameworkのSerialize/Deserializeの仕様を把握することである。
