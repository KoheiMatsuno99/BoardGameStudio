...
- 頻繁にコマの移動を行うため、操作がスムーズに行えるようこと
- Webアプリケーションにおいて典型的なセキュリティの脆弱性（SQLインジェクション、CSRF, XSSなど）をうまないようにすること
- 機能追加やバグ修正が容易な設計・実装であること
- 十分なテストが可能なコードとアーキテクチャを設計することで、バグの早期発見や修正が容易であること
...
そして、設計全体のポイントは各クラスが直列な関係にあるという点である。

つまり、TableクラスにBlockクラスの二次元配列をメンバー変数として持つことで、**各コマの位置情報をTableクラスが行列形式で一元管理できる。**
これにより、**コマの位置情報の取得や更新が直感的で簡潔な実装**になった。

![image](https://github.com/Amnis333/BoardGameStudio/assets/83019007/2d888841-9dbb-46e3-9829-2003febfd5a9)
